...
- (void)viewDidLoad {
  ...
  self.navigationItem.title = [NSString
      stringWithFormat:@"%@ %@ %@", [_postObj[kPostsTypeKey] uppercaseString],
                       @"For", [_postObj[@"company"] uppercaseString]];

  [MegaphoneUtility
      containsUserInBackground:_postObj
                  relationType:kRelationFollowers
                         block:^(BOOL contains, NSError *error) {
                           if (contains) {
                             [_favoriteButtonOutlet
                                 setImage:[UIImage imageNamed:@"ios7-star-1"]
                                 forState:UIControlStateNormal];
                           } else {
                             [_favoriteButtonOutlet
                                 setImage:[UIImage
                                              imageNamed:@"ios7-star-outline-1"]
                                 forState:UIControlStateNormal];
                           }
                         }];
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [MegaphoneUtility
      containsUserInBackground:_postObj
                  relationType:kRelationLikers
                         block:^(BOOL contains, NSError *error) {
                           if (contains) {
                             [_upButton
                                 setImage:[UIImage
                                              imageNamed:@"ios7-arrow-up-green"]
                                 forState:UIControlStateNormal];
                           }
                         }];
...
- (void)scrollViewForKeyboard:(NSNotification *)aNotification up:(BOOL)up {
  ...
  [self.view
      setFrame:CGRectMake(self.view.frame.origin.x,
                          self.view.frame.origin.y +
                              (keyboardFrame.size.height * (up ? -1 : 1)),
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (cell == nil) {
    cell =
        [[CommentTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
  ...
  [MegaphoneUtility
      containsUserInBackground:comment
                  relationType:kRelationLikers
                         block:^(BOOL contains, NSError *error) {
                           if (contains) {
                             [cell.likeButtonOutlet
                                 setImage:[UIImage
                                              imageNamed:@"ios7-arrow-up-green"]
                                 forState:UIControlStateNormal];
                           } else {
                             [cell.likeButtonOutlet
                                 setImage:[UIImage imageNamed:@"ios7-arrow-up"]
                                 forState:UIControlStateNormal];
                           }
...
- (IBAction)upButtonPressed:(id)sender {
  [MegaphoneUtility containsUserInBackground:_postObj
                                relationType:kRelationLikers
                                       block:^(BOOL contains, NSError *error) {
                                         if (contains) {
...
- (IBAction)actionSheetPressed:(id)sender {
  BOOL isAuthor =
      [[PFUser currentUser].objectId isEqualToString:_postObj[@"usernameId"]];
...
- (void)reportOrDeletePost {
  ...
  if (isAuthor) {
    ...
    [MegaphoneUtility
        containsUserInBackground:_postObj
                    relationType:kRelationReporters
                           block:^(BOOL contains, NSError *error) {
                             if (contains) {
                               UIAlertView *alert = [[UIAlertView alloc]
                                       initWithTitle:@"Post Reported"
                               ...
                               [MegaphoneUtility reportPostInBackground:_postObj
                                                                  block:nil];
                               UIAlertView *alert = [[UIAlertView alloc]
                                       initWithTitle:@"Post Reported"
                                             message:@"This post was "
...
- (void)sharePost {
  NSString *type = [_postObj[kPostsTypeKey] isEqualToString:@"other"]
                       ? @"Post"
                       : _postObj[kPostsTypeKey];

  NSString *message = [NSString
      stringWithFormat:@"Help me support this %@: %@ for %@", type,
                       _postObj[kPostsTitleKey], _postObj[@"company"]];
  NSURL *site =
      [NSURL URLWithString:@"https://www.google.com/search?q=megaphone"];

  UIActivityViewController *activityViewController = [
      [UIActivityViewController alloc] initWithActivityItems:@[ message, site ]
...
- (void)changeToLiked {
  [_upButton setImage:[UIImage imageNamed:@"ios7-arrow-up-green"]
             forState:UIControlStateNormal];
  [MegaphoneUtility likePostInBackground:_postObj
                                   block:^(BOOL succeeded, NSError *error) {
                                     _countLabel.text =
                                         [_postObj[@"numLikes"] stringValue];
                                   }];
}

- (void)changeToUnliked {
  [_upButton setImage:[UIImage imageNamed:@"ios7-arrow-up"]
             forState:UIControlStateNormal];
  [MegaphoneUtility unlikePostInBackground:_postObj
                                     block:^(BOOL succeeded, NSError *error) {
                                       _countLabel.text =
                                           [_postObj[@"numLikes"] stringValue];
                                     }];
...
- (IBAction)postComment:(id)sender {
  if ([_commentTextField.text isEqual:@""]) {
    UIAlertView *alert =
        [[UIAlertView alloc] initWithTitle:@"Missing comment"
...
- (void)changeToFollowed {
  [_favoriteButtonOutlet setImage:[UIImage imageNamed:@"ios7-star-1"]
                         forState:UIControlStateNormal];
  [MegaphoneUtility
      followPostInBackground:_postObj
                       block:^(BOOL succeeded, NSError *error) {
                         _followersCountLabel.text =
                             [_postObj[kPostsNumFollowersKey] stringValue];
                       }];
}

- (void)changeToUnfollowed {
  [_favoriteButtonOutlet setImage:[UIImage imageNamed:@"ios7-star-outline-1"]
                         forState:UIControlStateNormal];
  [MegaphoneUtility
      unfollowPostInBackground:_postObj
                         block:^(BOOL succeeded, NSError *error) {
                           _followersCountLabel.text =
                               [_postObj[kPostsNumFollowersKey] stringValue];
                         }];
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)gestureRecognizer {
  if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
    ...
    if (indexPath) {
      ...
      BOOL isAuthor =
          [[PFUser currentUser]
                  .objectId isEqualToString:pressed_comment[@"usernameId"]];
      ...
      if (isAuthor) {
        ...
        alert =
            [[UIAlertView alloc] initWithTitle:@"Report this comment?"
                                       message:@"Is this comment innapropriate "
                                               @"or against our guidelines?"
                                      delegate:self
                             cancelButtonTitle:@"Cancel"
...
- (void)reportOrDeleteComment {
  ...
  if (isAuthor) {
    ...
    [MegaphoneUtility
        containsUserInBackground:pressed_comment
                    relationType:kRelationReporters
                           block:^(BOOL contains, NSError *error) {
                             if (contains) {
                               UIAlertView *alert = [[UIAlertView alloc]
                                       initWithTitle:@"Comment Reported"
                                             message:@"You have already "
                               ...
                               [MegaphoneUtility
                                   reportCommentInBackground:pressed_comment
                                                       block:nil];
                             }
                           }];
  }
