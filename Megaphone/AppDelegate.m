...
- (BOOL)application:(UIApplication *)application
  ...
  // Initialize Parse's Facebook Utilities singleton. This uses the
  // FacebookAppID we specified in our App bundle's plist.
...
- (BOOL)application:(UIApplication *)application
  ...
  if ([[url host] isEqualToString:@"company"]) {
    ...
    UINavigationController *nav =
        (UINavigationController *)tabController.selectedViewController;
    PFObject *companyObj = [MegaphoneUtility getCompanyObject:[url path]];
    if (companyObj) {
      PostsTableViewController *postTableVC = [storyboard
          instantiateViewControllerWithIdentifier:@"postTableViewController"];
    ...
    if (postObj) {
      PFObject *companyObj =
          [MegaphoneUtility getCompanyObject:postObj[kPostsCompanyIdKey]];
      PostsTableViewController *postTableVC = [storyboard
          instantiateViewControllerWithIdentifier:@"postTableViewController"];
      postTableVC.companyObj = companyObj;
      [nav pushViewController:postTableVC animated:YES];
      PostViewController *postVC = [storyboard
          instantiateViewControllerWithIdentifier:@"postViewController"];
...
// Sent to the delegate to determine whether the log in request should be
    ...
    self.window.rootViewController = [[UIStoryboard
        storyboardWithName:@"Main"
                    bundle:
                        [NSBundle
                                mainBundle]] instantiateInitialViewController];
    //        NSLog([[UIStoryboard storyboardWithName:@"Main" bundle:[NSBundle
    //        mainBundle]] instantiateInitialViewController]);
  } else {
    NSLog(@"Not logged in");

    // Create the log in view controller
    MLoginViewController *logInViewController =
        [[MLoginViewController alloc] init];
